...
def report(
    ...
    # НАЧАЛО НОВЫЕ ПАРАМЕТРЫ (2.3.2)
    title: str = typer.Option("EDA-отчёт", help="Заголовок отчёта."),
    top_k_categories: int = typer.Option(10, help="Количество топ-значений для категорий."),
    # КОНЕЦ НОВЫЕ ПАРАМЕТРЫ (2.3.2)
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        # НАЧАЛО НОВЫЕ ФЛАГИ
        f.write(f"- Дубликаты строк: **{quality_flags['duplicates_count']}** (Флаг: {quality_flags['has_full_duplicates']})\n")
        f.write(f"- Константные колонки: **{quality_flags['constant_columns_count']}** (Флаг: {quality_flags['has_constant_columns']})\n\n")
        # КОНЕЦ НОВЫЕ ФЛАГИ 
