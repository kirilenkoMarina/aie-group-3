...
def compute_quality_flags(df: pd.DataFrame, summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # НАЧАЛО НОВЫЕ ЭВРИСТИКИ (2.3.1)
    # Колонки c одинаковыми значениями
    constant_cols_count = (df.nunique() <= 1).sum()
    flags["has_constant_columns"] = constant_cols_count > 0
    flags["constant_columns_count"] = int(constant_cols_count)

    # Полные дубликаты строк
    duplicates_count = df.duplicated().sum()
    flags["has_full_duplicates"] = duplicates_count > 0
    flags["duplicates_count"] = int(duplicates_count)
    # КОНЕЦ НОВЫЕ ЭВРИСТИКИ (2.3.1)
    ...
    # НАЧАЛО НОВЫЙ СКОР (шифруем дубликаты и константные колонки)
    if flags["has_constant_columns"]:
        score -= 0.1
    if flags["has_full_duplicates"]:
        score -= 0.1
    # КОНЕЦ НОВЫЙ СКОР (шифруем дубликаты и константные колонки)
